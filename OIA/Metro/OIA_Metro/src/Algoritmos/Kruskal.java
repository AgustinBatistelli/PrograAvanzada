...
public class Kruskal {
	...
	public static int kruskal(GrafoNoDirigidoPonderado grafo) {
		...
		while ( seguir && it.hasNext()){ ///La cantidad de aristas tiene que ser igual a la cantidad de nodos menos uno
			Arista arista = it.next();
			if (encontrarPadre(arista.getNodo1(), padres) != encontrarPadre(arista.getNodo2(), padres)) { ///Si tienen el mismo padre, están conectadas
	...
	private static void inicializarPadres(int[] padres) { ///UNION FIND
		for (int i = 0; i < padres.length; i++) {
			padres[i] = i;
		}
	}

	private static int encontrarPadre(int nodo, int[] padres){///UNION FIND - BUSQUEDA
